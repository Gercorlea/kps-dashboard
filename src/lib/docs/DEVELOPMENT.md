...
| `npm run commit` | commit guiado con commitizen |

## Git y commits

- **Los commits los hace únicamente Rauf, a mano.** Los agentes de código
  (Claude, etc.) dejan los cambios en el working tree y avisan; nunca
  ejecutan `git commit` ni `git push`.

- Se trabaja en ramas por feature (`feat/…`, `fix/…`) y se integra con PR.
- Convención de commits: **Conventional Commits** (`feat:`, `fix:`, `docs:`,
  ...
  bloqueo de archivos .env).
- `package-lock.json` va commiteado; el gestor de esta rama es **npm**.
