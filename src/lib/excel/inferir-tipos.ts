// Inferencia de esquema sobre una hoja cruda: dónde está el encabezado, qué tipo
// tiene cada columna y cómo leer los números y fechas que vienen como texto.
//
// Módulo puro y sin imports de runtime: toda la lógica delicada del feature vive
// aquí y se puede probar sin DOM ni navegador.
...
// Tras quitar moneda y espacios, un valor numérico sólo puede tener dígitos y separadores.
...
export function detectarOrdenFecha(valores: string[]): 'dia-mes' | 'mes-dia' {
  ...
  // Sin evidencia, dd/mm: convención es-MX.
  return mesDia > diaMes ? 'mes-dia' : 'dia-mes'
}

/**
 * Siempre `new Date(y, m - 1, d)`, jamás `new Date('2024-01-05')`: la forma
 * string se interpreta como medianoche UTC y en un huso negativo corre cada
 * fecha un día, moviendo las ventas de fin de mes al mes equivocado.
 */
...
/**
 * Los exports de SAP traen título y rango de fechas antes del encabezado real.
 * Devuelve el índice 0-based de la fila de encabezado, o -1 si no hay ninguna
 * (en cuyo caso los datos empiezan en la fila 0 y los nombres se sintetizan).
 */
...
export function valorNumerico(v: CeldaCruda, col: MetaColumna): number | null {
  ...
  // Un número suelto NUNCA se interpreta como fecha serial de Excel y viceversa:
  // el heurístico "entre 25569 y 60000 seguro es una fecha" convierte importes
  // en fechas. read-excel-file ya devuelve Date cuando la celda tiene formato
  // de fecha, así que no hay nada que adivinar.
  return null
}
...
const RE_METRICA = /importe|total|venta|monto|valor|neto|bruto|precio|subtotal|ingreso/i
const RE_DIMENSION =
  /cliente|producto|art[ií]culo|vendedor|categor[ií]a|sucursal|zona|regi[oó]n|canal|familia|marca|tienda|almac[eé]n/i
...
export function elegirDimension(columnas: MetaColumna[]): number {
  ...
  const porNombre = candidatas.find((c) => c.tipo === 'categoria' && RE_DIMENSION.test(c.nombre))
  if (porNombre) return porNombre.indice
